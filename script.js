...
function updateBasket(){ $('#inquiryCount').textContent=basket.length; $('#floatingCount').textContent=basket.length; $('#basketItems').innerHTML=basket.length?basket.map(i=>`<div class="basket-row"><span>${i.name}</span><b>${i.qty} pcs</b><button data-remove="${i.id}">Remove</button></div>`):'<p>Your inquiry basket is empty.</p>'; }
