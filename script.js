...
function openProduct(id){ const p=products.find(x=>x.id==id); $('#modalContent').innerHTML=`<div class="modal-product"><div class="product-img" style="--grad:${p.grad}">${p.icon}</div><div><p class="eyebrow">${p.intention} Energy</p><h2>${p.name}</h2><p>${p.desc}</p><p><b>Crystal:</b> ${p.crystal}</p><p><b>Chakra:</b> ${p.chakra}</p><p><b>Zodiac:</b> ${p.zodiac}</p><p><b>Shape:</b> ${p.shape}</p><p><b>MOQ:</b> ${p.moq} pcs</p><p><b>OEM Options:</b> custom logo, card, packaging, gift box and mixed sets.</p><button class="btn primary" data-add="${p.id}">Get Free Quote</button></div></div>`; showModal('productModal'); }
...
function updateBasket(){ const inc = $('#inquiryCount'); if(inc) inc.textContent=basket.length; const flc = $('#floatingCount'); if(flc) flc.textContent=basket.length; $('#basketItems').innerHTML=basket.length?basket.map(i=>`<div class="basket-row"><span>${i.name}</span><b>${i.qty} pcs</b><button data-remove="${i.id}">Remove</button></div>`).join(''):'<p>Your inquiry basket is empty.</p>'; }
...
/* --- Shop by Intention background image override --- */
const intentionBackgrounds = [
  'assets/images/intent-01.png',
  'assets/images/intent-02.png',
  'assets/images/intent-03.png',
  'assets/images/intent-04.png',
  'assets/images/intent-05.png',
  'assets/images/intent-06.png',
  'assets/images/intent-07.png',
  'assets/images/intent-08.png'
];
intentions.forEach((item, index) => item.image = intentionBackgrounds[index] || '');
renderIntentions = function(){
  $('#intentGrid').innerHTML = intentions.map(i => `
    <article class="intent-card" data-filter="intention" data-value="${i.name}">
      <div class="intent-img" style="background-image:url('${i.image}');" aria-hidden="true"></div>
      <h3>${i.name}</h3>
      <p>${i.desc}</p>
    </article>
  `).join('');
...
/* =========================================================
   ...
   Purpose: Keep each section's images independent.
   Do not put Shop by Intention background images inline.
   Images are assigned by CSS classes in assets/css/background-config.css.
   ========================================================= */
renderIntentions = function(){
  $('#intentGrid').innerHTML = intentions.map((i, index) => `
    <article class="intent-card" data-filter="intention" data-value="${i.name}">
      <div class="intent-img intent-bg-${index + 1}" aria-hidden="true"></div>
      <h3>${i.name}</h3>
      <p>${i.desc}</p>
    </article>
  `).join('');
...
/* =========================================================
   Popular Crystal Products background override
   First 5 uploaded images are used as card/modal backgrounds.
   ========================================================= */
...
renderProducts = function(list){
  $('#productGrid').innerHTML = list.map(p => {
    const bgClass = productBackgroundMap[p.id] || '';
    const usePhoto = !!bgClass;
    return `<article class="product-card"><div class="product-img ${usePhoto ? `product-photo ${bgClass}` : ''}" style="${usePhoto ? '' : `--grad:${p.grad}`}">${usePhoto ? '' : p.icon}</div><h3>${p.name}</h3><p>${p.intention} • ${p.chakra} Chakra<br>${p.crystal} • MOQ ${p.moq} pcs</p><div class="card-actions"><button data-view="${p.id}">View Details</button><button data-add="${p.id}">Get Free Quote</button></div></article>`;
...
openProduct = function(id){
  ...
  const bgClass = productBackgroundMap[p.id] || '';
  const usePhoto = !!bgClass;
  $('#modalContent').innerHTML = `<div class="modal-product"><div class="product-img ${usePhoto ? `product-photo ${bgClass}` : ''}" style="${usePhoto ? '' : `--grad:${p.grad}`}">${usePhoto ? '' : p.icon}</div><div><p class="eyebrow">${p.intention} Energy</p><h2>${p.name}</h2><p>${p.desc}</p><p><b>Crystal:</b> ${p.crystal}</p><p><b>Chakra:</b> ${p.chakra}</p><p><b>Zodiac:</b> ${p.zodiac}</p><p><b>Shape:</b> ${p.shape}</p><p><b>MOQ:</b> ${p.moq} pcs</p><p><b>OEM Options:</b> custom logo, card, packaging, gift box and mixed sets.</p><button class="btn primary" data-add="${p.id}">Get Free Quote</button></div></div>`;
