...
openProduct = function(id){
  ...
  const bgClass = productBackgroundMap[p.id] || '';
  const usePhoto = !!bgClass;
  $('#modalContent').innerHTML = `<div class="modal-product"><div class="product-img ${usePhoto ? `product-photo ${bgClass}` : ''}" style="${usePhoto ? '' : `--grad:${p.grad}`}">${usePhoto ? '' : p.icon}</div><div><p class="eyebrow">${p.intention} Energy</p><h2>${p.name}</h2><p>${p.desc}</p><p><b>Crystal:</b> ${p.crystal}</p><p><b>Chakra:</b> ${p.chakra}</p><p><b>Zodiac:</b> ${p.zodiac}</p><p><b>Shape:</b> ${p.shape}</p><p><b>MOQ:</b> ${p.moq} pcs</p><p><b>OEM Options:</b> custom logo, card, packaging, gift box and mixed sets.</p><button class="btn primary" data-add="${p.id}">Add to Inquiry</button></div></div>`;
